...
pool.on('connect', () => {
  if (!isProduction && !hasLoggedPoolConnection) {
    hasLoggedPoolConnection = true;
    logger.debug('Conectado a la base de datos PostgreSQL');
  }
});
...
const query = async (text, params) => {
  ...
  try {
    ...
    if (!isProduction && duration >= slowQueryThresholdMs) {
      logger.debug('Query ejecutado', { query: text, duration, rows: res.rowCount });
    }
    return res;
  } catch (error) {
...
const getClient = async () => {
  ...
  const timeout = setTimeout(() => {
    logger.warn('Cliente ha estado checked out por más de 5 segundos');
  }, 5000);
