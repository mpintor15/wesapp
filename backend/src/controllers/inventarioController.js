...
const inventarioCommandRepository = require('../repositories/inventario/inventarioCommandRepository');
...
const getUbicaciones = async (req, res) => {
  try {
    const result = await inventarioReadRepository.findUbicacionesSimple();
...
const getBajasArticulos = async (req, res) => {
  try {
    ...
    const result = await inventarioReadRepository.findBajasArticulos({
      search,
      from,
      to,
      pagination,
...
const exportMovimientosExcel = async (req, res) => {
  try {
    ...
    const result = await inventarioReadRepository.findMovimientosForExport({
      from,
      to,
      destino_id,
    });
...
const exportBajasArticulosExcel = async (req, res) => {
  try {
    const { search, from, to } = req.query;
    validateInventoryDateBounds(from, to);
    const result = await inventarioReadRepository.findBajasArticulos({ search, from, to });
...
const generateMovimientoPdf = async (movimientoId) => {
  const detalles = await inventarioReadRepository.findMovimientoDetalleForPdf(movimientoId);
  if (!detalles.length) {
    return null;
  }
...
const createMovimiento = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (pdfResult) {
        await inventarioCommandRepository.updateMovimientoPdfPath(
          movimientoId,
...
const downloadMovimientoPdf = async (req, res) => {
  try {
    const id = parsePositiveInteger(req.params.id, 'El id del movimiento es inválido');
    const result = await inventarioReadRepository.findMovimientoPdfPath(id);
    if (result.rowCount === 0) {
...
const regenerateMovimientoPdf = async (req, res) => {
  ...
  try {
    const id = parsePositiveInteger(req.params.id, 'El id del movimiento es inválido');
    const result = await inventarioReadRepository.movimientoExists(id);
