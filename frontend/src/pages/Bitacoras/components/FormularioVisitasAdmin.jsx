...
const FormActions = ({ form, canGestionar, onEdit, onArchive }) =>
  canGestionar && form.estado === 'ACTIVE' ? (
...
const FormularioVisitasAdmin = ({
  ...
  useEffect(() => {
    ...
    bitacorasService.getFormularioVisitasActivo(ubicacionId).then((result) => {
      ...
      if (result.success) {
        ...
        setTiposVisita(tipos.map((tipo) => tipo.nombre));
        setTipoDraft('');
        setFields(
  ...
  const addTipoVisita = () => {
    ...
    if (!tipo || tiposVisita.some((existing) => existing.toLowerCase() === tipo.toLowerCase()))
      return;
    setTiposVisita((current) => [...current, tipo]);
    setTipoDraft('');
  };

  const removeTipoVisita = (tipo) => {
    setTiposVisita((current) => current.filter((item) => item !== tipo));
    setFields((current) =>
      current.map((field) =>
        Array.isArray(field.aplica_a)
                  ...
                  {tiposVisita.map((tipo) => (
                    <li key={tipo}>
                      ...
                      <button
                        ...
                        aria-label={`Quitar tipo de visita ${tipo}`}
                        onClick={() => removeTipoVisita(tipo)}
                        disabled={saving}
                      >
                        Quitar
                      </button>
              ...
              {loadingTemplate
                ...
                : fields.map((field, index) => (
                    <fieldset className="bitacoras-form-field-row" key={field.draftId}>
                      ...
                      <p className="bitacoras-field-type-help">
                        {FIELD_TYPES.find(([value]) => value === field.type)?.[2]}
                      </p>
                      <div className="form-group bitacoras-applies-input">
                        ...
                        {field.aplica_a !== 'TODOS' ? (
                          tiposVisita.length ? (
                            ...
                            >
                              {tiposVisita.map((tipo) => (
                                <label key={tipo} className="bitacoras-checkbox-field">
                                  <input
                                    type="checkbox"
                                    checked={field.aplica_a.includes(tipo)}
                                    onChange={() => toggleFieldTipo(field, tipo)}
                                  />
                                  {tipo}
                                </label>
                              ))}
